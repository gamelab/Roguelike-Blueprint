...
// loot and traps
PlayState.lootList;
PlayState.extraLootList;
PlayState.trapList;
PlayState.lootMap;
PlayState.extraLootMap;
PlayState.trapMap;
...
PlayState.initEntities = function () {
	...
	for (var i = 0; i < 5; i++)
	{
		...
		// add references to the actor to the extra loot list & map
	...
	for (var p = 0; p < 1; p++)
	{
		...
		{
			if(rand<1/2) {
				extraItem.type = this.extraLootTypes.WAND;
			} else {
				extraItem.type = this.extraLootTypes.POTION;
			}
		}
		else if(this.isCrypt)
		{
		...
		{
			if(rand<1/2) {
				extraItem.type = this.extraLootTypes.PICKAXE;
			} else {
				extraItem.type = this.extraLootTypes.PICKAXE;
			}
		}
		...
		{
			// pick a random position that is both a floor and not occupied
			extraItem.y = this.randomInt(this.mapSize);
			extraItem.x = this.randomInt(this.mapSize);
		}
		while (this.getTile(extraItem.x,extraItem.y) != '.' || this.extraLootMap[extraItem.x + "_" + extraItem.y] != null);


		// add references to the actor to the extra loot list & map
		this.extraLootMap[extraItem.x + "_" + extraItem.y] = extraItem;
		this.extraLootList.push(extraItem);
	}
	...
	for (var e = 0; e < this.numberOfActors; e++) {
		...
		} while (this.getTile(actor.x,actor.y) != '.' || this.actorMap[actor.x + "_" + actor.y] != null || this.trapMap[actor.x + "_" + actor.y] != null || this.lootMap[actor.x + "_" + actor.y] || this.extraLootMap[actor.x + "_" + actor.y] != null);
...
PlayState.initEntitySprites = function() {
	...
	// extra loot
	for (var p = 0; p < 1; p++) {
		var extraLoot = this.extraLootList[p];
		extraLoot.sprite = new Kiwi.GameObjects.Sprite(this, this.textures.extraLoot, extraLoot.x*this.tileSize, extraLoot.y*this.tileSize);

		// used the right sprite for each extra loot type
		if(extraLoot.type == this.extraLootTypes.WAND){
			extraLoot.sprite.cellIndex = 0;
		} else if(extraLoot.type == this.extraLootTypes.PICKAXE){
			extraLoot.sprite.cellIndex = 1;
		} else {
			extraLoot.sprite.cellIndex = 2;
		}

		this.addChild(extraLoot.sprite);
	}
...
// draw all entities: actors, traps, loot, extra loot, etc on top of the ascii map
PlayState.drawEntities = function() {
	...
	// draw extra loot
	for (var p in this.extraLootList) {
		if (this.extraLootList[p] != null) {
			var symbol = '';
			if(this.extraLootList[p].type == this.extraLootTypes.WAND){
				symbol = '2';
			} else if(this.extraLootList[p].type == this.extraLootTypes.PICKAXE) {
				symbol = '3';
			} else if(this.extraLootList[p].type == this.extraLootTypes.POTION)  {
				symbol = '4';
			}

			this.setCell(this.extraLootList[p].x, this.extraLootList[p].y, symbol);
		}
	}
	...
	for (var a in this.actorList) {
		if (this.actorList[a] != null && this.actorList[a].hp > 0) {
			...
			if(a == 0){
				...
				symbol = 'd';
			} else if(this.actorList[a].ai == Kiwi.Plugins.AI.AiTypes.TREANT)  {
				// random enemies: treant
				symbol = 't';
			}
...
PlayState.initScreen = function() {
	...
	// we have 10 types of tiles in our tile map
	for(var t=0; t<=10; t++) {
	...
	for (var x = 0; x < this.mapSize; x++) {
		...
		for (var y = 0; y < this.mapSize; y++) {
			...
			if(this.isDungeon) {
				...
				if(tile=='.') {
					mapLayer.setTile(x,y,8+1);
				} else if (tile=='#') {
					// we have 2 crypt wall types just for visual varitey
					if(Math.random()<0.5) {
						mapLayer.setTile(x,y,9+1);
					} else {
						mapLayer.setTile(x,y,10+1);
					}
				}
			} else {
				if(tile=='.') {
...
PlayState.moveTo = function(actor, dir) {
	...
	if (self.actorMap[newKey] != null) {
		...
		// pick up extra loot
		var extraLoot = self.extraLootMap[actor.x + '_' + actor.y];
		if(extraLoot!=null && actor.isPlayer) {
			// remove picked up extra loot from the map
			self.extraLootMap[actor.x + '_' + actor.y] = null;
			self.extraLootList[self.extraLootList.indexOf(extraLoot)]=null;
			self.removeChild(extraLoot.sprite);

			// apply effects of extra loot
			self.pickUpExtraLoot(extraLoot, actor);
		}
...
PlayState.pickUpLoot =  function(loot, picker) {
	...
	//	}
		...
		picker.shots = 4;
		//if you already have a wand
		if (picker.shotsBar) {
			picker.shotsBar.counter.current = picker.shots++;
		} else {
		picker.shotsBar = new Kiwi.HUD.Widget.IconBar(this.game, this.textures.shot, picker.shots, picker.shots, 340 , 224);
		this.game.huds.defaultHUD.addWidget(picker.shotsBar);
		}
	}
}

// apply effects of picked up extra loot
PlayState.pickUpExtraLoot =  function(extraLoot, picker) {
	var self = PlayState;
	...
	} else if(extraLoot.type==self.extraLootTypes.PICKAXE) {
		// enable digging
		picker.pickaxe = extraLoot.sprite;
		picker.pickaxe.x = self.mapSize*self.tileSize + 10;
		picker.pickaxe.y = 416;
		if(picker.isPlayer) {
			this.addChild(picker.pickaxe);
	...
	} else if(extraLoot.type==self.extraLootTypes.WAND) {
		// get 4 magic missle shots
		picker.shots = 4;
		//if you already have a wand
		if (picker.shotsBar) {
	...
	//What I want to do is have the number of shots you currently have be represented by a number. This didn't really work (and I'm not
	//totally sure why?) but that's why this section is commented out. This ties into the playerBolts thing, and explains why THAT is
	//commented out.

	//} else if(loot.type==self.LootTypes.WAND) {
	//	 get 4 magic missle shots
	//	picker.shots = 4;
	//	picker.shotsBar = new Kiwi.HUD.Widget.Icon(this.game, this.textures.shot,
	//		picker.shots, picker.shots, 340 , 224);
	//	picker.shotsNum = new Kiwi.HUD.Widget.TextField(this.game, this.player.shots, 340 , 256);
	//	this.game.huds.defaultHUD.addWidget(picker.shotsBar);
	//	this.game.huds.defaultHUD.addWidget(picker.shotsNum);
	//}
}
...
PlayState.toggleGraphics = function() {
	if(this.graphicsButton.cellIndex == 1) {
		...
		// extra loot
		for (var p=0; p < this.extraLootList.length; p++) {
			if(this.extraLootList[p]!=null) {
				this.extraLootList[p].sprite.visible = false;
			}
		}
		// traps
		...
		for (var p=0; p < this.extraLootList.length; p++) {
			if(this.extraLootList[p]!=null) {
				this.extraLootList[p].sprite.visible = true;
			}
		}
		// traps
		for (var t=0; t < this.trapList.length; t++) {
...
//	this.player.shotsNum.counter.current = 0;
	...
	this.actorMap = null;
	this.lootList = null;
	this.extraLootList = null;
	this.trapList = null;
	this.lootMap = null;
	this.extraLootMap = null;
	this.trapMap = null;
	this.left = null;
	this.right = null;
	this.up = null;
